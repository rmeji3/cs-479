...
export default function Home() {
  ...
  const distColor =
    dist === -1               ? 'text-zinc-400' :
    dist <= DISTANCE_DANGER_MM  ? 'text-red-600'    :
    dist <= DISTANCE_WARNING_MM ? 'text-orange-600' :
    dist <= BLIND_SPOT_THRESHOLD_MM ? 'text-yellow-600' : 'text-zinc-950';

  const spo2Color =
    spo2 === 0       ? 'text-zinc-400'  :
    spo2 < SPO2_LOW  ? 'text-red-600'   :
    spo2 < 97        ? 'text-yellow-600': 'text-zinc-950';

  const impactColor =
    accelMag >= FALL_THRESHOLD ? 'text-red-600'    :
    accelMag >= 15             ? 'text-orange-600' :
    accelMag > 0               ? 'text-zinc-950'   : 'text-zinc-400';

  function fmtDist(d: number): string {
    if (d <= 0) return '—';
    // convert mm to ft (≥1 ft) or in (<1 ft)
    return d >= 304.8 ? (d / 304.8).toFixed(1) : (d / 25.4).toFixed(1);
  }
  const distUnit = dist > 0 ? (dist >= 304.8 ? 'ft' : 'in') : undefined;

  return (
    <div className="h-screen bg-white text-zinc-950 flex flex-col overflow-hidden select-none">

      {/*  Header  */}
      <header className="flex items-center justify-between px-4 h-11 border-b border-zinc-200 shrink-0">
        ...
        <div className="flex items-center gap-3">
          <Link
            ...
            className="flex items-center gap-1.5 text-xs text-zinc-600 hover:text-zinc-900
                       border border-zinc-300 hover:border-zinc-400 rounded-lg px-2.5 py-1 transition-colors"
          >
            <Settings size={11} />
            Settings
          </Link>
          {serial.status === 'connected' ? (
            <div className="flex items-center gap-2.5">
              ...
              <button
                ...
                className="text-[11px] text-zinc-600 hover:text-red-600 border border-zinc-300
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs text-zinc-600 hover:text-zinc-900
                         border border-zinc-300 hover:border-zinc-400 rounded-lg px-3 py-1 transition-colors"
            >
              <PlugZap size={12} />
              Connect Device
            </button>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="w-[42%] grid grid-cols-2 grid-rows-2 gap-px bg-zinc-200 shrink-0">

          {/* Distance */}
          <div className={cn('bg-white', blindSpotAlert && 'bg-orange-100 transition-colors duration-500')}>
            <MetricTile
              label="Distance"
              value={fmtDist(dist)}
              unit={distUnit}
          ...
          {/* Impact */}
          <div className={cn('bg-white', accelMag >= FALL_THRESHOLD && 'bg-red-100 transition-colors duration-500')}>
            <MetricTile
              label="Impact"
              value={accelMag === 0 ? '—' : (accelMag * 3.28084).toFixed(1)}
              unit={accelMag !== 0 ? 'ft/s²' : undefined}
      ...
      {/*  Footer  */}
      <footer className="h-8 flex items-center justify-between px-4 border-t border-zinc-200 shrink-0 text-[10px] text-zinc-600 font-mono">
        <span>
